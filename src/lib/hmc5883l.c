...
// sensor board front direction (NED x axis) is the ldo regulator side,
// opposite of the silkscreen y direction
...
void hmc5883L_GetValues(float* px, float* py, float* pz) {
    s16 x,y,z;
    hmc5883L_GetRawData(&x, &y, &z);
    *px = (float)(myBias_ - y) * myScale_;
    *py = (float)(mxBias_ - x) * mxScale_;
    *pz = (float)(mzBias_ - z) * mzScale_;
    }
